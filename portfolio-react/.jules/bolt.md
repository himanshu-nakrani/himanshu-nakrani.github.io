...
## 2026-05-28 - Combining sequential useMemo and avoiding manual push loops
**Learning:** When multiple `useMemo` hooks depend strictly on each other's sequential output in a React component, keeping them separate adds unnecessary internal hook tracking overhead, dependency array checking, and intermediate allocations during renders. Furthermore, manually flattening arrays using `.push()` inside `for...of` loops as a micro-optimization is less efficient in modern JS engines (like V8) compared to the array spread operator (`[...a, ...b]`), which pre-calculates the required array size and allocates memory once.
**Action:** Combine dependent `useMemo` hooks into a single hook to reduce React overhead, and prefer the array spread operator for flattening known arrays rather than dynamic `.push()` calls that trigger multiple expensive memory reallocations.
