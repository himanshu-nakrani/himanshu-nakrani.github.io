...
## 2024-05-06 - Accessible Exit Transitions
**Learning:** Found components (e.g., BackToTop button) abruptly unmounting with `if (!visible) return null`, which breaks smooth CSS exit transitions.
**Action:** When a component needs an exit transition, keep it in the DOM and use `aria-hidden={!visible}`, `tabIndex={visible ? 0 : -1}`, and `pointerEvents: visible ? 'auto' : 'none'` to make it semantically and functionally invisible without breaking the animation.
