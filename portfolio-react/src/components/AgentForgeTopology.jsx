  ...
  return (
    <section className="signature-visual topology" aria-labelledby="topology-title">
      ...
      <div className="topology__canvas">
        <svg viewBox="0 0 880 300" role="group" aria-label="Agent Forge system topology">
          ...
          {nodes.map(node => <g key={node.id} className={`topology__node${active.id===node.id?' is-active':''}`} role="button" aria-pressed={active.id===node.id} tabIndex="0" aria-label={`${node.label}: ${node.detail}`} onClick={()=>setActive(node)} onKeyDown={event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();setActive(node)}}}><circle cx={node.x} cy={node.y} r="29"/><text x={node.x} y={node.y+48} textAnchor="middle">{node.label}</text><text className="topology__node-index" x={node.x} y={node.y+4} textAnchor="middle">{String(nodes.indexOf(node)+1).padStart(2,'0')}</text></g>)}
        </svg>
      </div>
