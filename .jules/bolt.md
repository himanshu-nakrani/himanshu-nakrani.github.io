...
## 2024-09-01 - Avoid chained array methods for finding maximums in high-frequency callbacks
**Learning:** Using chained array methods like `.filter().sort()[0]` inside a high-frequency callback (such as an `IntersectionObserver` on scroll) introduces O(N log N) overhead and triggers unnecessary intermediate array allocations, creating garbage collection pressure and potentially slowing down the scroll performance.
**Action:** Always replace chained filtering and sorting for finding a maximum value with a single-pass `for...of` loop inside high-frequency event listeners and observer callbacks. This achieves O(N) performance and avoids temporary array allocations entirely.
